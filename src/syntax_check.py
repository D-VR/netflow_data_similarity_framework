...
def remove_syntax_erros_files(load_path, store_path, model):
    ...
    for file in files:
        if '.txt' in file or '.csv' in file:
            print(file)
            path = load_path+file
            df = pd.read_csv(path, usecols=COLUMNS_TARGET, on_bad_lines='skip', quoting=csv.QUOTE_NONE, nrows=10000, header=0)
